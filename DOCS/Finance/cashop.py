...
def fetch_and_flatten(data_url):
    try:
        ...
        ref_codes_list = []
        for entry in data:
            operation_id = entry.get("operation_id") or entry.get("id")
        ...
        print(f"✅ Получено: {len(main_df)} операций, {len(ref_codes_df)} ref-кодов")

        df_dict = {
            name: df for name, df in {
                "CashOP_operations": main_df,
                "CashOP_refcodes": ref_codes_df
...
if __name__ == "__main__":
    DATA_URL = "https://smartup.online/b/anor/mxsx/mkcs/cash_operation$export"

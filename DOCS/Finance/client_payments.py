...
def fetch_and_flatten(data_url):
    try:
        ...
        for entry in data:
            return_id = entry.get("deal_id") or entry.get("movement_id") or entry.get("visit_id")
        ...
        df_dict = {
            name: df for name, df in {
                "clientpayments_return": return_df,
                "clientpayments_returnproducts": return_products_df,
                "clientpayments_details": details_df
...
if __name__ == "__main__":
    DATA_URL = "https://smartup.online/b/trade/txs/tcs/cashin$export"

...
def fetch_and_flatten(data_url):
    try:
        ...
        for entry in data:
            visit_id = entry.get("deal_id") or entry.get("visit_id")  # на случай если нет deal_id
        ...
        # Подтаблица: details (внутри return_products)
        details_list = []
        ...
        # Убираем пустые DataFrame или без колонок
        df_dict = {

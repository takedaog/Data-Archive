...
def fetch_and_flatten(data_url):
    try:
        ...
        # Таблица перемещений
        movements_df = pd.json_normalize(data, sep="_", max_level=1)

        # Таблица movement_items
        movement_items = []
        for movement in data:
            movement_id = movement.get("movement_id")
            for item in movement.get("movement_items", []):
                item["movement_id"] = movement_id
                movement_items.append(item)
        movement_items_df = pd.DataFrame(movement_items)

        print(f"✅ Получено: {len(movements_df)} перемещений, {len(movement_items_df)} товаров")

        df_dict = {
            name: df for name, df in {
                "movement": movements_df,
                "movement_items": movement_items_df
...
if __name__ == "__main__":
    DATA_URL = "https://smartup.online/b/anor/mxsx/mfm/movement$export"
